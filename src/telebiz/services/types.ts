...
export interface Subscription {
  ...
  // Per-organization subscription model: seats within this org
  total_seats: number; // Calculated from actual members + pending invitations
  paid_seats: number; // Seats that will be charged (after free seat discount)
  free_seats: number; // Free seats (every 5th seat is free)
  billed_seats: number; // What Paddle is currently billing for
  max_seats: number; // -1 for unlimited
  seats_mismatch: boolean; // True if billed_seats != paid_seats (needs sync)
}
...
export interface PlanLaunchOffer {
...
export interface PlanVolumePricing {
  monthly: {
    price_per_user: number;
  };
  annual: {
...
export interface Plan {
  ...
  monthly?: PlanPricing;
  annual?: PlanPricing;
  launch_offer?: PlanLaunchOffer;
  volume_pricing?: PlanVolumePricing;
...
export interface PricingTiers {
  volume_threshold: number;
  small: {
    monthly: { price_per_user: number };
    annual: { price_per_user: number; total_per_user: number };
  };
  volume: {
    monthly: { price_per_user: number };
    annual: { price_per_user: number; total_per_user: number };
...
export interface CanCreateOrgResponse {
  can_create: boolean;
  reason?: string;
}
